 ...
 * Kept here so both auth screens stay DRY and consistent.
 */
export function googleAuthErrorMessage(raw: string): string {
  const m = (raw || '').toLowerCase()
  if (m.includes('not enabled') || m.includes('unsupported provider') || m.includes('provider is not'))
    return 'Google Sign-In is not configured yet. Enable Google provider in Supabase Auth settings.'
  ...
  if (m.includes('redirect'))
    return 'Google sign-in could not redirect back to MUSVORA. Please try again.'
  if (m.includes('network') || m.includes('fetch') || m.includes('load failed'))
    return 'Connection error reaching Google. Check your internet and try again.'
...
export function GoogleButton({ label, loading = false, disabled = false, onClick }: GoogleButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      {loading ? <Loader2 size={18} className="animate-spin" aria-hidden /> : <GoogleMark />}
