...
function MusicCard({ song, variant = 'default' }: MusicCardProps) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <p className={`text-sm font-medium truncate ${isActive ? 'text-white' : 'text-zinc-100'}`}>
              {song.title}
            </p>
            {song.human_verified && (
        ...
        {isActive && (
          <div className="flex gap-0.5 items-end h-4" aria-hidden aria-label="Now playing">
            {[1, 2, 3].map(i => (
              <div
                key={i}
                className={`w-0.5 rounded-full ${isCurrentPlaying ? 'animate-pulse bg-white' : 'bg-white/40'}`}
  ...
  return (
    <article
      className="rounded-2xl border border-white/10 overflow-hidden cursor-pointer group transition-all duration-200 hover:bg-white/8 hover:border-white/20 will-change-transform"
    ...
    >
      <div className="relative aspect-square overflow-hidden">
        ...
        <button
          onClick={handlePlay}
          aria-label={isCurrentPlaying ? `Pause ${song.title}` : `Play ${song.title}`}
          className="absolute bottom-3 right-3 w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-xl opacity-0 group-hover:opacity-100 transition-all hover:scale-105 focus:opacity-100 focus:scale-105"
        >
          {isCurrentPlaying
            ? <Pause size={16} strokeWidth={2.5} aria-hidden />
            : <Play size={16} strokeWidth={2.5} fill="black" aria-hidden />
          }
        </button>
        {song.human_verified && (
          <div
