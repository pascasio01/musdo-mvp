...
export default function Market() {
  ...
  const filtered = mockLicenses.filter(l => {
    ...
    const matchesFilter = activeFilter === 'All'
      || l.license_type.replace('-', ' ').toLowerCase() === activeFilter.toLowerCase().replace('-', ' ')
  ...
  return (
    <AppShell>
      <div className="px-5 pt-14">
        <div className="mb-6">
          <p className="text-muted text-xs uppercase tracking-widest font-semibold mb-1">License</p>
          <h1 className="text-primary text-3xl font-black">Marketplace</h1>
          <p className="text-muted text-sm mt-1">Human-verified songs ready to license.</p>
        </div>
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3.5 rounded-2xl border border-theme text-primary text-sm outline-none transition-colors placeholder-zinc-600"
            style={{ background: 'var(--glass-bg)' }}
          />
        </div>

        <div className="flex gap-2 overflow-x-auto pb-1 mb-6 scrollbar-hide" role="group" aria-label="Filter licenses">
        ...
        <div className="space-y-4 pb-6">
          {filtered.map(license => {
            ...
            return (
              <article
                key={license.id}
                className="rounded-3xl border border-theme overflow-hidden transition-colors duration-[var(--speed,300ms)] hover:border-glass"
                style={{ background: 'var(--glass-bg)' }}
              >
                <div className="flex items-center gap-4 p-4 pb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <h3 className="text-primary font-bold truncate">{song?.title}</h3>
                      {song?.human_verified && <VerificationBadge type="human_verified" size="xs" />}
                      {song?.verified_rights_holder && <VerificationBadge type="verified_rights_holder" size="xs" />}
                    </div>
                    <p className="text-muted text-sm truncate">{song?.artist_name}</p>
                    <p className="text-muted text-xs mt-0.5">{song?.genre}</p>
                  </div>
                </div>
                ...
                <div className="px-4 py-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-primary font-black text-xl">${license.price}</span>
                    <button
                      onClick={() => setSelectedLicense(license)}
                      className="px-4 py-2.5 rounded-2xl text-sm font-bold transition-opacity hover:opacity-90"
                ...
                {song?.bpm && (
                  <div className="px-4 pb-4 flex gap-4 text-[11px] text-muted">
          ...
          {filtered.length === 0 && (
            <div className="py-16 text-center">
              <p className="text-muted text-sm">No licenses match your search.</p>
            </div>
          )}
        </div>
      </div>
